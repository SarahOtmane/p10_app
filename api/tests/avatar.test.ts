  ...
  describe('Query', () => {
    describe('getAllAvatars', () => {
      it('should return all avatars', async () => {
        (Avatar.findAll as jest.Mock).mockResolvedValue([{ id_avatar: 1 }]);
      ...
      it('should throw error on DB error', async () => {
        (Avatar.findAll as jest.Mock).mockRejectedValue(new Error('DB error'));
  ...
  describe('Mutation', () => {
    describe('addAvatar', () => {
      it('should add avatar successfully', async () => {
        ...
        (Avatar.create as jest.Mock).mockResolvedValue({ picture_avatar: 'test.png' });
      ...
      it('should throw error on fs or DB error', async () => {
        (fs.ensureDir as jest.Mock).mockRejectedValue(new Error('FS error'));
        const input = {
          filename: 'test.png',
          base64Image: 'data:image/png;base64,aGVsbG8=',
        };
    ...
    describe('updateAvatar', () => {
      it('should update avatar successfully', async () => {
        (Avatar.findByPk as jest.Mock).mockResolvedValue({ id_avatar: 1 });
        (Avatar.update as jest.Mock).mockResolvedValue([1]);
      ...
      it('should throw error if avatar does not exist', async () => {
        (Avatar.findByPk as jest.Mock).mockResolvedValue(null);
        await expect(
      ...
      it('should throw error on DB error', async () => {
        (Avatar.findByPk as jest.Mock).mockRejectedValue(new Error('DB error'));
        await expect(
          (avatarResolvers.Mutation as any).updateAvatar(
            null,
            { id_avatar: 1, picture_avatar: 'new.png' },
    ...
    describe('deleteAvatar', () => {
      it('should delete avatar successfully', async () => {
        (Avatar.findByPk as jest.Mock).mockResolvedValue({ id_avatar: 1 });
        (Avatar.destroy as jest.Mock).mockResolvedValue(1);
    ...
    describe('replaceUserAvatar', () => {
      it('should replace user avatar successfully', async () => {
        (Avatar.findByPk as jest.Mock).mockResolvedValue({ id_avatar: 2 });
